...
def run(cmd, desc=""):
    ...
    name = desc or os.path.basename(cmd.split()[1] if len(cmd.split()) > 1 else cmd)
    print(f"\n{'='*60}\n  {name}\n{'='*60}")
    r = subprocess.run(cmd, shell=True, cwd=SCRIPTS if not cmd.startswith('python scripts') else ROOT)
    if r.returncode != 0:
...
def run_pbmc_experiments():
    ...
    run('python run_pbmc.py', 'PBMC 3K (NB-LR vs Fisher z)')
    run('python run_synthetic.py', 'Synthetic Benchmarks (100 seeds)')
    run('python run_baselines.py', 'Baselines (NOTEARS, GENIE3)')

def run_full_experiments():
    """Run all experiments including cross-tissue validation."""
    run_pbmc_experiments()
    run('python run_everything.py', 'Full Sweep (all configs)')
    run('python run_paul.py', 'Paul15 Cross-Tissue')
    run('python run_fix30.py', 'Fixed d=30 + Cell Types')

def run_validation():
    """Run Reactome + DepMap validation."""
    run('python _download_reactome.py', 'Download Reactome Pathways')
    run('python _reactome_validate.py', 'Reactome Co-Membership Validation')
    run('python _depmap_coessential.py', 'DepMap CRISPR Co-Essentiality')

def generate_figures():
    """Generate all figures from results."""
    figs_dir = os.path.join(ROOT, 'scripts', 'gen_figures')
    for fig in ['gen_fig1.py', 'gen_fig2.py', 'gen_fig3.py', 'gen_fig4.py']:
        run(f'python {fig}', f'Figure: {fig}')
...
def verify_results():
    ...
    checks = {
        ...
        '_everything_ckpt.json': ['B_paul_d30', 'E_sim_d30', 'alpha_sweep'],
        'depmap_coessentiality.json': ['s100a8_s100a9'],
        'simulation_results.json': ['d30', 'd50', 'd100'],
    }
    all_ok = True
